...
Now, we will work with the `Professor Trash Wheel`data set. In the following code chunk we use a similar process as above to import, clean, and organize the data. 
...
Lastly, we combine 'Professor Trash Wheel' data set with the 'Mr. Trash Wheel' data set to produce a single tidy data set. To keep track of which Trash Wheel is which, you may added an additional variable to both data sets before combining. To combine the data we use the `bind_rows()` function from the dplyr package in R to bind together two data frames by their rows:

```{r}
# mutate(trash_wheel,
#          trash_dataset = c(1))
# 
# mutate(professor_wheel,
#          trash_dataset = c(2))
# 
#  trash_tidy = bind_rows(trash_wheel,professor_wheel)
```
